...
function run_ACDC_scopf_contigency_cuts(network::Dict{String,<:Any}, model_type::Type, run_scopf_prob::Function, check_contingency_violation::Function, optimizer, setting; max_iter::Int=100, time_limit::Float64=Inf)   
    ...
    #     end
    # end
    # for (i,shunt) in network_active["shunt"]
    #     if haskey(solution["shunt"], i)
    #         shunt["bs"] = solution["shunt"][i]["bs"]
    #     end
    # end
    ...
    while contingencies_found > 0
        ...
        skip_itr_flag = false
        if check_contingency_violation == check_contingency_violations
            contingencies = check_contingency_violation(network_base, model_type, optimizer, setting, contingency_limit=iteration,dominated_contingencies=dominated_contingencies)   
        elseif check_contingency_violation == check_contingency_violations_SI
        ...
        if contingencies_found <= 0 && contingencies.total_cuts_pre_filter < (iteration - 1)
        ...
        if check_contingency_violation == check_contingency_violations
            # reduce(append!,(dominated_contingencies,contingencies.dominated_contingencies))
        end   
        iteration += 1
    end
