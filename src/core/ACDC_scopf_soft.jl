...
function build_scopf_soft(pm::_PM.AbstractPowerModel)
    ...
    for i in _PM.ids(pm, nw=0, :convdc)                                                
        ...
        # constraint_dc_droop_control(pm, i, nw=0)
        constraint_dc_droop_control_linear(pm, i, nw=0)                                                                         
    end
    ...
    for nw in contigency_ids
        ...
        for i in _PM.ids(pm, nw=nw, :convdc)                                               
            ...
            # constraint_dc_droop_control(pm, i, nw=nw)
            constraint_dc_droop_control_slack_link(pm, i, nw_1=0, nw_2=nw)
            constraint_dc_droop_control_linear(pm, i, nw=nw)                                                                             
        end
