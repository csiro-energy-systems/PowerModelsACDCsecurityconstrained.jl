...
#
  ...
  function run_acdc_scopf_ptdf_dcdf_cuts_iterative(network::Dict{String,<:Any}, model_type::Type, run_scopf_prob::Function, optimizer; kwargs...)
    return build_acdc_scopf_ptdf_dcdf_cuts_iterative(network, model_type, run_scopf_prob, optimizer; kwargs...)
end

function build_acdc_scopf_ptdf_dcdf_cuts_iterative(network::Dict{String,<:Any}, model_type::Type, run_scopf_prob::Function, optimizer; max_iter::Int=1000, time_limit::Float64=Inf)
    if _IM.ismultinetwork(network)
        Memento.error(_LOGGER, "run_acdc_scopf_ptdf_dcdf_cuts_iterative can only be used on single networks")
    end
    ...
    while cuts_found > 0
        ...
        cuts = check_contingencies_branch_acdc_power(network, optimizer, total_cut_limit=iteration, gen_flow_cuts = network["gen_flow_cuts"], branch_flow_cuts = network["branch_flow_cuts"], branchdc_flow_cuts = network["branchdc_flow_cuts"])
        
        result_scopf["gen_contingencies_unsecure"] = cuts.gen_conts_unsecure
        result_scopf["branch_contingencies_unsecure"] = cuts.branch_conts_unsecure
        result_scopf["branchdc_contingencies_unsecure"] = cuts.branchdc_conts_unsecure
