...
data["area_gens"][1] = Set{Int64}()
data["area_gens"][2] = Set{Int64}()
data["area_gens"][3] = Set{Int64}()

for (i,gen) in data["gen"]
    bus_id = gen["gen_bus"]
    if data["bus"]["$bus_id"]["area"] == 1
        push!(data["area_gens"][1], parse(Int64, i))
    elseif data["bus"]["$bus_id"]["area"] == 2
        push!(data["area_gens"][2], parse(Int64, i))
    elseif data["bus"]["$bus_id"]["area"] == 3
        push!(data["area_gens"][3], parse(Int64, i))
    end
end
